...
This repository is the python implementation of the attractor identification method described in the following work:
...
* Find the conditionally stable motifs and their supports in plant-pollinator Boolean models. 
...
* The LDOI operations are performed on the expanded network and hence are in 'number index' notation.  
...
The first item is a dictionary of virtual nodes, i.e., the nodes and their states in that consistent cycle/conditionally stable motif, and the second item is a set of conditions with '~' showing the inactive state. It is stored in this format so that it would be easier to check if this set is a subset of LDOI of a set of node states. 
